...
description: 单只可转债历史日/周/月/年 K 线（convertible_bond_candlesticks，GET /api/v1/market/data/convertible-bond-candlesticks）。用户问某只可转债的日/周/月/年 K 线、开高低收、前/后复权时使用。必填 --symbol、--interval-unit、--since-ts-millis、--until-ts-millis；可选 --adjust-kind、--limit。分钟 K 线请改用 convertible-bond-minutes。
---
