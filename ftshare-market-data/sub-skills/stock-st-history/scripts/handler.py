...
BASE_URL=os.environ.get("FTSHARE_BASE_URL","https://market.ft.tech/gateway").rstrip("/"); ENDPOINT="/api/v1/market/data/stock-st-history"
...
def main():
 p=argparse.ArgumentParser(description="查询股票历史ST状态"); p.add_argument("--symbol",required=True); p.add_argument("--st-type"); a=p.parse_args()
 if not a.symbol.strip(): p.error("symbol 不能为空")
 params={"symbol":a.symbol}
 if a.st_type: params["st_type"]=a.st_type
