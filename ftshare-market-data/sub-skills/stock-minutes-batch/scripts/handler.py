...
BASE_URL=os.environ.get("FTSHARE_BASE_URL","https://market.ft.tech/gateway").rstrip("/");_REQUEST_HEADERS={"FTSHARE_API_KEY":os.environ.get("FTSHARE_API_KEY", ""),"Content-Type":"application/json"}; ENDPOINT="/api/v2/market/data/stock_minutes/batch"
def main():
 p=argparse.ArgumentParser(); p.add_argument("--symbols",required=True); p.add_argument("--since-ts-millis",required=True,type=int); p.add_argument("--until-ts-millis",required=True,type=int); p.add_argument("--interval-value",type=int,default=1); p.add_argument("--adjust-kind",choices=["none","forward","backward","None","Forward","Backward"],default="none"); p.add_argument("--limit",type=int,default=50); a=p.parse_args()
 ...
 if a.adjust_kind and a.adjust_kind.lower()!="none":params.append(("adjust_kind",a.adjust_kind.lower()))
